...
* `sam local start-api`: Inicializa um container que expõe a função Lambda a um gatilho de requisição HTTP por uma API. Essa API é inicializada por padrão na porta 3000, e pode ser acessada em `http://127.0.0.1:3000`. A função "hello world" pode ser invocada sem parâmetros pela chamada de API `curl http://localhost:3000/`, mas também é possível a passagem de dados para a função pelo corpo da requisição HTTP em um POST, por exemplo. Encerrar o terminal que está executando o comando `sam local start-api` também desligará o container Docker.
